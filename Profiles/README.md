...
- Built-in destinations: Home, Downloads, OneDrive, LocalAppData, ProgramData, Roaming (`%APPDATA%`), Temp, ProgramFiles, ProgramFilesX86, Windows, WinTemp (`C:\Windows\Temp`), plus three for admin work: IMELogs (the Intune Management Extension logs), StartMenu (all users') and PublicDesktop. Any whose folder doesn't exist on the machine is left out — IMELogs only appears on Intune-enrolled devices. The original five come first, so `j prog` still means ProgramData (`j files` / `j x86` for Program Files) and `j appdata` still means LocalAppData (`j roam` for Roaming). For favorites, prefer `j -Add`. For version-controlled literals use `config.psd1`'s `ExtraJumpFolders`; for anything needing evaluation (env vars, conditional paths) append in `Machines/<COMPUTERNAME>.ps1`: `$script:JumpFolders += [pscustomobject]@{ Label='VMs'; Path='D:\VMs' }`
