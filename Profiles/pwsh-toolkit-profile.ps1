...
if ($script:Config.Prompt -eq 'OhMyPosh') {
    $themePath = $script:Config.OhMyPoshTheme
    if ($themePath -and -not ([IO.Path]::IsPathRooted($themePath)) -and -not ($themePath -match '[\\/]')) {
        # Bare filename → resolve relative to Profiles/OhMyPosh/
        $themePath = Join-Path $script:ProfileRoot 'OhMyPosh' $themePath
    }
