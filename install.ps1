...
.PARAMETER Uninstall
    Cleanly back out: remove the $PROFILE entry (only if it's actually a
    pwsh-toolkit install — your own profile is left alone) and restore whatever
    profile install.ps1 had backed up, if any. Reports what it deliberately
    leaves (the repo, your config.psd1, installed tools, your secrets). Add
    -Purge to also delete the toolkit's machine-local caches.

.PARAMETER Purge
    Only with -Uninstall: also remove the machine-local caches the toolkit
    created — the Oh My Posh theme gallery (%LOCALAPPDATA%\pwsh-toolkit), the
    tip-rotation state (%LOCALAPPDATA%\PSProfile), and the archive-peek temp
    tree ($env:TEMP\peek). Does NOT remove installed tools or your secrets.
...
.EXAMPLE
    ...
    # Remove the profile entry (and restore a backed-up profile if there is one).

.EXAMPLE
    .\install.ps1 -Uninstall -Purge
    # ...and also delete the toolkit's caches (theme gallery, tip state, peek temp).
#>
...
# True if $profilePath is a pwsh-toolkit install (symlink to the loader, or a
# stub that dot-sources it). Used to decide whether install can replace it and
# whether uninstall may remove it — so we never delete a profile that isn't ours.
...
if ($Uninstall) {
    ...
    if (-not (Test-Path -LiteralPath $profilePath)) {
        Write-Host "No profile at $profilePath — nothing to remove there." -ForegroundColor DarkGray
    }
    ...
    elseif ($PSCmdlet.ShouldProcess($profilePath, 'Remove pwsh-toolkit profile')) {
        ...
        # Put back the profile install.ps1 replaced, if a backup exists.
        $leaf   = Split-Path -Leaf $profilePath
        $backup = Get-ChildItem -LiteralPath (Split-Path -Parent $profilePath) -Filter "$leaf.backup-*" -ErrorAction Ignore |
                  Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($backup) {
            Move-Item -LiteralPath $backup.FullName -Destination $profilePath -Force
            Write-Host "✓ Restored your previous profile from $($backup.Name)" -ForegroundColor Green
        }
    }

    # Optional: remove the machine-local caches the toolkit created.
    if ($Purge) {
        Write-Host ''
        Write-Host 'Purging toolkit caches...' -ForegroundColor Cyan
        $caches = @(
            @{ Path = (Join-Path $env:LOCALAPPDATA 'pwsh-toolkit'); What = 'Oh My Posh theme gallery + state' }
            @{ Path = (Join-Path $env:LOCALAPPDATA 'PSProfile');     What = 'tip-rotation state' }
            @{ Path = (Join-Path $env:TEMP 'peek');                  What = 'archive-peek temp tree' }
        )
        $found = @($caches | Where-Object { Test-Path -LiteralPath $_.Path })
        if ($found.Count -eq 0) {
            Write-Host '  (no caches found)' -ForegroundColor DarkGray
        } else {
            foreach ($c in $found) {
                if ($PSCmdlet.ShouldProcess($c.Path, "Remove ($($c.What))")) {
                    Remove-Item -LiteralPath $c.Path -Recurse -Force -ErrorAction SilentlyContinue
                    Write-Host "  ✓ Removed $($c.Path)" -ForegroundColor Green
                }
            }
        }
    }

    # What we deliberately leave alone, so "clean" is honest about its scope.
    Write-Host ''
    Write-Host 'Left untouched (delete manually if you want them gone):' -ForegroundColor DarkGray
    Write-Host '  - This repo and your Profiles/config.psd1 — just delete the cloned folder.' -ForegroundColor DarkGray
    Write-Host '  - oh-my-posh, the Meslo Nerd Font, and the Terminal-Icons module — shared tools the OMP setup installed.' -ForegroundColor DarkGray
    Write-Host '  - Your SecretStore secrets and any Windows Terminal font change.' -ForegroundColor DarkGray
    if (-not $Purge) {
        Write-Host '  - Toolkit caches (theme gallery, tip state, peek temp) — re-run with -Purge to remove these too.' -ForegroundColor DarkGray
    }
    Write-Host ''
    if ($removed) { Write-Host 'Uninstalled. Open a new shell for it to take effect.' -ForegroundColor Cyan }
    return
}
