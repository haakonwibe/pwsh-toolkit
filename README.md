...
| **`note` / `today` / `Find-Note`** | Lightweight markdown journal. `note "thing"` appends a timestamped bullet to `<NotesRoot>/YYYY-MM-DD.md`; `today` opens today's file in your default `.md` app (Obsidian-friendly — point `NotesRoot` at a vault subfolder). `Find-Note "term"` greps across every note. |
